...
const items: { icon: React.ElementType; label: string }[] = [{ icon: Home, label: "Inicio" }, { icon: Briefcase, label: "Servicos" }, { icon: CreditCard, label: "Planos" }, { icon: Headphones, label: "Suporte" }, { icon: User, label: "Conta" }]
    ...
    <>
      ...
      <aside className={"fixed top-0 left-0 h-full w-72 bg-background border-r z-50 transform transition-transform duration-300 shadow-2xl " + (open ? 'translate-x-0' : '-translate-x-full')}>
        <div className="flex items-center justify-between p-4 border-b">
          <span className="font-bold text-lg">Quotes to Scrape</span>
